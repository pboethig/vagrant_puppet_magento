...
enviroment=dev
##############################################################################################################################################################################################################################################################################
# some went wrong on first rsync
if [ ! -d "`iniconfig $enviroment filesystem documentroot`" ]
then
    echo  "=> magentofolder does not extist. Create now`iniconfig $enviroment filesystem documentroot`"
    mkdir -f /var/www/magento
    echo  "=> copy magento to webroot `iniconfig $enviroment filesystem documentroot`"
    sudo cp -fr /vagrant/workingbase/* /var/www/magento
fi
##############################################################################################################################################################################################################################################################################
#change to webroot to get n98-magerun in the right location
echo  "=> cd to `iniconfig $enviroment filesystem documentroot`"
