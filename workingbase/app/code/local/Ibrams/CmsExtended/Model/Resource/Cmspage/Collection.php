 ...
 */
/**
 * Cmspage collection resource model
 *
 * @category    Ibrams
 * @package     Ibrams_CmsExtended
 * @author      Ultimate Module Creator
 */
class Ibrams_CmsExtended_Model_Resource_Cmspage_Collection extends Mage_Core_Model_Resource_Db_Collection_Abstract
{
    ...
    protected function _renderFiltersBefore()
    {
        if ($this->getFilter('store')) {
            $this->getSelect()->join(
                array('store_table' => $this->getTable('ibrams_cmsextended/cmspage_store')),
                'main_table.entity_id = store_table.cmspage_id',
                array()
            )
     ...
     * @author Ultimate Module Creator
     */
    public function addIdFilter($cmspageIds)
    {
        if (is_array($cmspageIds)) {
            if (empty($cmspageIds)) {
                $condition = '';
            } else {
                $condition = array('in' => $cmspageIds);
            }
        } elseif (is_numeric($cmspageIds)) {
            $condition = $cmspageIds;
        } elseif (is_string($cmspageIds)) {
            $ids = explode(',', $cmspageIds);
            if (empty($ids)) {
                $condition = $cmspageIds;
            } else {
    ...
    protected function _toOptionArray($valueField='entity_id', $labelField='name', $additional=array())
    {
        ...
        foreach ($this as $item) {
            if ($item->getId() == Mage::helper('ibrams_cmsextended/cmspage')->getRootCmspageId()) {
                continue;
            }
    ...
    public function addProductFilter($product)
    {
        ...
        if (!isset($this->_joinedFields['product'])) {
            $this->getSelect()->join(
                array('related_product' => $this->getTable('ibrams_cmsextended/cmspage_product')),
                'related_product.cmspage_id = main_table.entity_id',
                array('position')
            );
    ...
    public function addCategoryFilter($category)
    {
        ...
        if (!isset($this->_joinedFields['category'])) {
            $this->getSelect()->join(
                array('related_category' => $this->getTable('ibrams_cmsextended/cmspage_category')),
                'related_category.cmspage_id = main_table.entity_id',
                array('position')
            );
